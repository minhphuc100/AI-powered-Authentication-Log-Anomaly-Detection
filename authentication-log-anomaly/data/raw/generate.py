...
for day in range(N_DAYS):
    ...
    for emp in active_employees:
        ...
        # --- Session 1: login đầu ngày (7h-9h) ---
        ...
        # --- Explicit logon rải trong buổi sáng (mở app cần quyền cao) ---
        n_explicit_morning = random.randint(1, 3)
        for _ in range(n_explicit_morning):
            t_explicit = login_time + timedelta(
                minutes=random.randint(10, 150)
            )
            if t_explicit.hour < 12:
                add_row(t_explicit, 4648, emp, ip_today, logon_type, ws)
 
        # --- Khóa máy đi ăn trưa rồi mở lại (session 2) ---
        # Logoff ~11h30-13h, login lại ~12h-13h30
        lunch_logoff = current_day.replace(
            hour=random.randint(11, 12), minute=random.randint(30, 59),
            second=random.randint(0, 59)
        )
        add_row(lunch_logoff, 4634, emp, ip_today, logon_type, ws)
 
        lunch_login = lunch_logoff + timedelta(minutes=random.randint(30, 75))
        # 5% khả năng gõ sai khi mở lại máy
        if random.random() < 0.05:
            add_row(lunch_login - timedelta(seconds=random.randint(5, 20)),
                    4625, emp, ip_today, logon_type, ws)
        add_row(lunch_login, 4624, emp, ip_today, logon_type, ws)
 
        # --- Explicit logon rải buổi chiều ---
        n_explicit_afternoon = random.randint(1, 4)
        for _ in range(n_explicit_afternoon):
            t_explicit = lunch_login + timedelta(
                minutes=random.randint(10, 200)
            )
            if t_explicit.hour < 19:
                add_row(t_explicit, 4648, emp, ip_today, logon_type, ws)
 
        # --- Đôi khi unlock máy giữa giờ làm (screen lock timeout) ---
        n_unlocks = random.randint(0, 3)
        for _ in range(n_unlocks):
            t_unlock = lunch_login + timedelta(
                minutes=random.randint(20, 240)
            )
            if t_unlock.hour < 18:
                add_row(t_unlock, 4624, emp, ip_today, "7", ws)
 
        # --- Logoff cuối ngày: 16h30 - 18h30 ---
    ...
    # SYSTEM noise mỗi ngày — tăng mật độ (service logon, scheduled task...)
